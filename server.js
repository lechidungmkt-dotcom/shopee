...
try {
    ...
    console.warn("⚠️ Không có biến môi trường RESEND_API_KEY, bỏ qua khởi tạo Resend.");
}
...
const server = http.createServer(async (req, res) => {
    ...
    // --- API: CONFIG (Cung cấp các cấu hình không nhạy cảm cho Frontend) ---
    if (req.url === '/api/config' && req.method === 'GET') {
        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({
            google_script_url: process.env.GOOGLE_APP_SCRIPT_URL || "",
            sepay_stk: process.env.SEPAY_ACCOUNT || "",
            sepay_bank: process.env.SEPAY_BANK || "MSB",
            sepay_name: process.env.SEPAY_ACCOUNT_NAME || ""
        }));
        return;
    }
